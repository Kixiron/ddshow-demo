 ...
 */
pub fn cmds_from_table_updates(
    prefix: &str,
    upds: Map<String, Value>,
) -> Result<Vec<UpdCmd>, String> {
    let mut commands = Vec::new();
    upds.into_iter().try_for_each(|(table, updates)| {
        cmds_from_table_update(prefix.to_string() + table.as_str(), updates, &mut commands)
    })?;
    Ok(commands)
}
...
fn record_into_field(rec: Record) -> Result<Value, String> {
    match rec {
        ...
        Record::Array(CollectionKind::Map, v) => {
            let elems: Result<Vec<Value>, String> = v
                .into_iter()
                .map(|x| match x {
        ...
        _ => Err(format!(
            "Cannot convert record field {:?} to JSON value",
            rec
        )),
    }
}
